...
select job_id from emp e where e.emp_id = 120;
...
select emp_id, dept_id from emp where emp_id = 115;
...
where salary < (select avg(salary) from emp where job_id = "ST_CLERK") 
				having (job_id <> "ST_CLERK" or job_id is null);
...
-- subquery where 절의 비료 컬럼(loc)가 unique가 아닐 경우 => 결과 행이 여러 개일 수 있다.
-- 다중행 subquery 연산
...
;
-- subquery에서 main 볃교dml table을 사용하도록 정의하면 상관 쿼리.
