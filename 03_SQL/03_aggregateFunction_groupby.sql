...
- 문자타입/일시타입: max(), min(), count()에만 사용가능
	- 문자열 컬럼의 max(): 사전식 배열에서 가장 마지막 문자열, min()은 첫번째 문자열. 
		- 특수문자 > 숫자 > 대문자 > 대문자 > 한글 순으로 큰 값을 가짐.
        - '10000' '600' 비교 ? 문자열일 경우 첫 문자열부터 순차적으로 크기 확인 후 정렬.
        - 위 경우 맨 처음 문자 '1' < '6' 이므로 '600'이 더 큰 값으로 판별됨.
...
select sum(salary),
	   round(avg(salary)),  -- round()와 많이 사용됨.
       min(salary),
       max(salary),
       stddev(salary),
       variance(salary),
...
select count(distinct ifnull(dept_name, "미배치"))
from emp;
