...
select e.emp_id, e.emp_name, e.salary,
	   d.dept_name, d.loc
from emp e left outer join dept d on e.dept_id = d.dept_id	# null값은 제외하고 join 된 것을 확인
order by 4;
...
--  직원_id(emp.emp_id)가 100, 110, 120, 130, 140인 직원의
-- ID(emp.emp_id),이름(emp.emp_name), 업무명(job.job_title) 을 조회.
...
-- count(*): 행 수를 카운트. 그래서 직원이 없는 부서도 1이 나옴.
-- 직원 수를 세는 것이므로 emp_id를 기준으로 count.
select d.dept_id,
	   d.dept_name,
